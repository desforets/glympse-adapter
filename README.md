...
- `viewer_handler_instance` is a class instance to handle events and messages
  generated by the Glympse Viewer (described below).
...
- The iframed viewer is created when calling the GA instance's `runHost()` method,
  with configuration.
...
- `events`: Additional events to be used for updates coming from the Glympse viewer:
  - `StateUpdate`: Called whenever something interesting has changed with the observed
  invite (more details, below).
  - `ViewerInit`: Handles when the Glympse viewer has started up it's viewing sequence,
  but is still not quite ready for interaction
  - `ViewerReady`: Called when the Glympse viewer is fully initialized and ready for
  interaction. It should be noted that the only *required* parameter in the above
  configuration is the `url` parameter, which initializes the Glympse viewer iframe.

All API calls return a Promise, allowing for async handling of all results. These
should be treated like any baseline [Promise/A+] system:
...
will include [Cordova] integration for interfacing with hybrid clients, where the client
"app" is the set of Cordova interfaces into a native implementation over a pure
Javascript solution.

The Cordova-aware version will also leverage the Glympse Cordova plug-in (if available)
to provide better Glympse/Cards Platform integration, as required.
...
provided on the cards.* endpoint for true messaging/full-duplex features and support.
